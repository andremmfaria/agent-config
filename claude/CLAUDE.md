...
`settings.json` sets `CLAUDE_CODE_EFFORT_LEVEL=auto`. The environment variable takes precedence over the `effortLevel` setting and the `--effort` flag, and `auto` means every model uses its own default effort instead of a forced level: haiku gets no effort param (so it spawns), while opus and sonnet still get their model defaults. This works on direct `api.anthropic.com` as well as third-party providers. Run `claude/apply-settings.sh` to inject this `env` block and the `permissions.allow` list (incl. `Edit(~/.claude/projects/**/memory/**)` so auto-memory edits never prompt) into `~/.claude/settings.json` (merged, not overwritten).
