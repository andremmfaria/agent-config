# USER.md - About Your Human

- **Timezone:** Europe/Dublin

## What They Care About

Describe technical, personal, and communication preferences here without adding private secrets or sensitive operational details.

## Communication Style

- Short, precise messages.
- Dense > padded.
- One recommendation when a decision is needed.

## What Works

- Research first, synthesis second.
- Flagging important issues unprompted.
- Direct disagreement when the premise is wrong.

## What Does Not Work

- Filler.
- Repeating the user's words back to them.
- Over-explaining familiar topics.
