...
| --- | --- | --- | --- |
| `block-destructive-bash.sh` (PreToolUse/Bash) | `before_tool_call`, matcher `exec`/`process`/`code_execution` | destructive-exec guard | Claude's `ask` maps to OpenClaw's `requireApproval` (`severity: "warning"`, `timeoutBehavior: "deny"`); Claude's `deny` maps to `{ block: true, blockReason }`. Regexes are byte-for-byte ports (`[[:space:]]` -> `\s`, `\b` unchanged) — see `rules.js` `EXEC_DENY_RULES` / `EXEC_ASK_RULES`. **Policy: ask only on really destructive commands; deny on catastrophic/exfil/gate-tampering.** ASK tier (both platforms, byte-for-byte): `rm -r`/`rm -f`, `find ... -delete`/`-exec rm`/`-execdir rm`, `git reset --hard`, `git clean -f/-d/-x`, `git checkout -- <path>`/`git restore`, `truncate`/`shred`, `curl\|wget \| sh/bash/zsh` (pipe-to-shell), `git push --force`/`--force-with-lease`/`-f`. Everything that used to ask and no longer does — `sudo`, package installs (pip/npm/pnpm/yarn/cargo/gem/go/brew/apt), `chmod +x`/`chmod 7xx`, `crontab`, `systemctl ...`, plain (non-force) `git push`, `gh pr/issue ...`, `gh release`, HTTP write-verb calls (`curl -X POST/PUT/PATCH/DELETE`, `--data`/`-d`/`-F`/`--json`) — is now an ordinary silent apply operation (the exfil DENY rule still catches any of these if they reference a sensitive path). DENY tier is unchanged: catastrophic `rm -rf` on root/home/parent, disk-format/raw-device-write/fork-bomb, recursive chmod/chown on `/`, writes to protected paths, credential/secret exfiltration. |
| `write-path-guard.sh` (PreToolUse/Write\|Edit\|MultiEdit\|NotebookEdit) | `before_tool_call`, matcher `write`/`edit`/`apply_patch` | write-path guard | Same protected-path set, `deny`/`ask` -> `block`/`requireApproval`. "cwd" becomes `ctx.workspaceDir` (falls back to `process.cwd()`); "`/tmp/claude-*` scratchpad" allowance is kept (subagents still write there) plus `os.tmpdir()`, gated by the `workspaceOnlyWrites` config flag. **Ask condition narrowed**: a target outside cwd/workspace/scratchpad only asks if the file already exists (overwriting it is the destructive case); creating a brand-new file out there can't clobber anything and is silent. `checkWritePathGuard` takes a `fileExists` boolean (caller-determined via `fs.existsSync`, kept out of the pure function) to make this decision. |
...
```json5
{
  ...
  workspaceOnlyWrites: true, // false = shrink the in-scope set to just cwd/workspace (drop the /tmp/claude-*  + os.tmpdir() scratchpad allowance)
}
```

`workspaceOnlyWrites` only controls which directories count as "in scope" (cwd/workspace, plus — when `true`, the default — `/tmp/claude-*` and `os.tmpdir()`). It does **not** control whether an out-of-scope write asks: that is decided separately by whether the target file already exists (`fileExists`, see the table above) — creating a brand-new file outside the workspace never asks, overwriting one that already exists there always does, regardless of this flag.
