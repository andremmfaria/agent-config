...
**Not blockers:** missing edge cases, stylistic preferences, "could be clearer", architectural opinions, code quality concerns, performance, security (unless explicitly broken).

If a plan blindly feeds untrusted web, repo, issue, email, log, or attachment content into tools/actions without a boundary or approval step, treat that as an execution blocker.
